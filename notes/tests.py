...
from notes.models import FreeContent, PaidContent
...
class FreeContentTestCase(APITestCase):
    ...
    def setUp(self):
        self.user = CustomUser.objects.create(
            username="Alice Test", phone_number="89015148266"
        )
    ...
    def test_free_content_create(self):
        self.user = CustomUser.objects.create(
            username="Alice Test 2", phone_number="89015148265"
        )
        self.client.force_authenticate(user=self.user)
        data = {
...
class PaidContentTestCase(APITestCase):
    ...
    def setUp(self):
        self.user = CustomUser.objects.create(
            username="Alice 3 Test", phone_number="89015148266", subscription=True
        )
        self.client.force_authenticate(user=self.user)
